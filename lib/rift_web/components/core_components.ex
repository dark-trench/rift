defmodule RiftWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: RiftWeb.Gettext
  ...
  def button(%{rest: rest} = assigns) do
    ...
    variants = %{
      "primary" => "#{base} bg-slate-950 text-white hover:bg-slate-800",
      nil => "#{base} border border-slate-300 bg-white text-slate-950 hover:bg-slate-50"
    }
    ...
    <div class="mb-4">
      <label>
        ...
        <select
          ...
          class={[
            @class ||
              "w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 shadow-sm",
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(RiftWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(RiftWeb.Gettext, "errors", msg, opts)
    end
  end
